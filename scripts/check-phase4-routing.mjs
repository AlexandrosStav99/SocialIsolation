...
const discovery=fs.readFileSync("lib/routing/discovery.ts","utf8"); const safety=fs.readFileSync("lib/safety/router.ts","utf8"); const matrix=fs.readFileSync("lib/safety/validated-matrix.ts","utf8"); const content=fs.readFileSync("lib/safety/content.ts","utf8"); const checklist=fs.readFileSync("docs/SAFETY-CONTENT-VALIDATION-CHECKLIST.md","utf8");
...
for(const x of ["Not production validated","requires_domain_expert_validation","qualified safeguarding / domain expert"]) if(!checklist.includes(x)) throw new Error("Safety validation gate documentation missing: "+x);
